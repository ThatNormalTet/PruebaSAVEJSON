...
def JSONThings():
    if command[1] == "makefile":
        try:
            ...
            print(
                "The operation could not be completed due to lack of arguments"
            )
        ...
        try:
            with open(f"{command[2]}.json", "w") as jsonfile:
                try:
                    ...
                    print(
                        "The operation could not be completed due to lack of arguments: jsonformattedcontent"
                    )
                    print(
                        "Command should look like: json writefile <filename> <jsonformattedcontent>"
                    )
        except IndexError:
            print(
                "The operation could not be completed due to lack of arguments: filename, jsonformattedcontent"
            )
            print(
                "Command should look like: json writefile <filename> <jsonformattedcontent>"
            )
...
def RemoveData():
    try:
        ...
        print(
            "The operation could not be completed due to lack of arguments: tag"
        )
...
def CommandAct():
    ...
    if command[0] == "help":
        ...
        print(
            "add data <jsonformattedtext> | Replaces the datapools list with a json formatted text"
        )
        print(
            "rem data <tag> | Removes the specified tag from the datapools list (Only works with top tags)"
        )
        print(
            "save data | Saves the datapools list into a json formatted file called 'data.json'"
        )
        print(
            "print data | Prints the current state of datapools list in the terminal"
        )
        print(
            "json makefile <filename> | Creates a .json file of the specified filename"
        )
        print("json remfile <filename> | Deletes the .json file specified")
        print(
            "json writefile <filename> <jsonformattedtext> | Creates a .json file of the specified name and writes the specified json formatted text"
        )
